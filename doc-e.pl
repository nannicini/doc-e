...
for (1 .. $n) {
  ...
  if ($pid) {
    ...
    until($dieNowChild){
        sleep(60);
        logEntry("CHILD PID ($$) run...");
        logEntry("RUN CMD ");
    }    
    logEntry("Child ($$) exiting");
    exit;
  }
}
...
until ($dieNow) {
	sleep($sleepMainLoop);
  logEntry("Loop Parent retrieve information from DB and script execution");
  logEntry("Forks:".$forks);
  # to do procedure to stop all child
}
